...
def update_on_time_delivery_rate(po_instance):
    if po_instance.status == 'completed':
        ...
        on_time_delivery_rate = completed_purchases / total_completed_purchases if total_completed_purchases > 0 else 0
...
def update_quality_rating_avg(po_instance):
    if po_instance.status == 'completed' and po_instance.quantityRating is not None:
        ...
        total_ratings = sum([purchase.quantityRating for purchase in completed_purchases])
        quality_rating_avg = total_ratings / completed_purchases.count() if completed_purchases.count() > 0 else 0
...
def update_average_response_time(po_instance):
    if po_instance.acknowledgementDate is not None:
        ...
        total_response_time_seconds = sum([(purchase.issueDate-purchase.acknowledgementDate).total_seconds() for purchase in response_times])
        total_response_time_days = total_response_time_seconds / (60 * 60 * 24)  # Convert seconds to days
        average_response_time_days = total_response_time_days / response_times.count() if response_times.count() > 0 else 0
...
def update_fulfillment_rate(po_instance):
    ...
    fulfillment_rate = fulfilled_purchases.count() / total_fulfilled_purchases.count() if total_fulfilled_purchases.count() > 0 else 0
...
def create_historical_performance(po_instance):
    if po_instance.status == 'completed':
        ...
        # Check if a historical performance entry already exists for the vendor
